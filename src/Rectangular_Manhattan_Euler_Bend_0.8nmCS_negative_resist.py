...
N = 50  # 阵列波导数
Ra = 607.157  # 罗兰圆半径，FPR长度，单位um
da = 6.4  # 阵列波导输入端口间隔
gap = 0.18  # 阵列波导孔径处间隙
w = 4 - 0.080  # 阵列波导宽度，多模波导
w0 = 1.2  # 单模波导宽度
Ni = 1  # 输入波导数
No = 16  # 输出波导数
wi = 3  # 输入波导开口宽度
...
Lis = 100  # 输入单模波导
Los = 100  # 输出单模波导
La = 40  # 阵列波导端口弯曲锥形波导长度
Lt = 30  # 单模过渡多模线性锥形波导长度
# 弯曲波导参数
R1 = 300  # 阵列波导孔径处矫正弯曲半径，普通多模弯曲
R2 = 90  # 90度直角欧拉弯曲
Ri = 60  # 输入端弯曲波导
Ro = 60  # 输出端弯曲波导
m_taper = 3
...
for ii in range(1, No+1):
    ...
    wg_o_overlay[ii-1] = AWG << gf.components.straight(length=10.0, width=1.2,
                                                       cross_section=gf.cross_section.strip(width=1.2, layer=(4, 2))).copy()
    wg_o[ii-1].move((-290, -250-(ii-1)*35))
    wg_o_overlay[ii-1].move((-290, -250-(ii-1)*35))
...
AWG.show()
# AWG.write_gds(r"E:\桌面\AWG最终版\GDS文件\AWG_0.8nmCS_16CH_0nmOS.gds")
