...
delta_L = m*lambda_c/neff  # 阵列波导总长度差
da = 6  # 阵列波导中心间距
theta_offset = deg2rad(30)  # 星形耦合器倾斜角度
...
while True:
    ...
    L_0 = 2 * (L[0] + R[0] * theta[0])  # 阵列波导第一根总长度
    if D > 10000:
        raise ValueError(f"This structure is wrong: D={D} is too large.")
    ...
    for i in range(1, Narray):
        ...
        test_R = R[i] - R[i-1]  # 检测R参数
        test_L = L[i] - L[i-1]  # 检测L参数
        if abs(test_R) > 100 or abs(test_L) > 100:
            raise ValueError(f"This structure is wrong.")
        if R[i] > 0 and L[i] > 0:
            raise ValueError(f"This structure is true: R[0]={R[0]}")
