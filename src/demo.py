...
# 绘制layer(1, 0)层布线
wg_i = AWG << gf.components.straight(length=10.0, width=6,
                                     cross_section=gf.cross_section.strip(width=6, layer=(4, 1))).copy()
wg_i.move((600, -130))
route_input = gf.routing.route_single(
    AWG,
    port1=wg_i.ports["o1"],
    port2=wg_i_port[0].ports["i1"],
    cross_section=gf.cross_section.strip(width=6, layer=(4, 1)),
    bend=gf.components.bend_circular,
    radius=50,  # 单位 μm
    layer=(4, 1)
)
# 绘制套刻层布线
wg_i_overlay = AWG << gf.components.straight(length=10.0, width=6,
                                             cross_section=gf.cross_section.strip(width=6, layer=(4, 2))).copy()
wg_i_overlay.move((600, -130))
route_input_overlay = gf.routing.route_single(
    AWG,
    port1=wg_i_overlay.ports["o1"],
    port2=wg_i_port[0].ports["i2"],
    cross_section=gf.cross_section.strip(width=6, layer=(4, 2)),
    bend=gf.components.bend_circular,
    radius=50,  # 单位 μm
    auto_taper=False
)
...
for ii in range(1, No+1):
    wg_o[ii-1] = AWG << gf.components.straight(length=10.0, width=6,
                                               cross_section=gf.cross_section.strip(width=6, layer=(4, 1))).copy()
    wg_o_overlay[ii-1] = AWG << gf.components.straight(length=10.0, width=6,
                                                       cross_section=gf.cross_section.strip(width=6, layer=(4, 2))).copy()
    wg_o[ii-1].move((-200, -200-(ii-1)*35))
    wg_o_overlay[ii-1].move((-200, -200-(ii-1)*35))
