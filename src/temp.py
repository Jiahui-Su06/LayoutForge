...
L1 = 100  # 第一段直波导长度
R1 = 100  # 第一段弯曲波导半径
L2_0 = 550  # 第二段直波导长度，初始值
R2 = 100  # 第二段弯曲波导半径
L3_0 = 50  # 第三段直波导长度，初始值
R3 = 100  # 第三段弯曲波导半径
L4_0 = 100  # 第四段直波导长度，初始值
...
sector1 = mydraw.sector(radius=Ra,
                        theta_start=rad2deg(-da * (Narray/2 - 1/2) / Ra - theta_buffer),
                        theta_stop=rad2deg(da * (Narray/2 - 1/2) / Ra + theta_buffer),
                        x_center=0, y_center=0,
                        angle_resolution=1.0, layer=(1, 0))
AWG.add_ref(sector1)
theta_span = dr * (N_channels/2 - 1/2) * 2 / Ra
sector2 = mydraw.sector(radius=Ra/2,
                        theta_start=180 - rad2deg(theta_span + theta_buffer),
                        theta_stop=180 + rad2deg(theta_span + theta_buffer),
...
for i in range(1, Narray + 1):
    ...
    # 阵列波导左侧部分第一段直波导，需要设计为锥形波导
    x_span = array_width  # taper短边宽度
    y_span_buffer = array_width
    x_span_taper = da + (da - x_span) / L1 * y_span_buffer  # taper长边宽度，需要延伸到星形耦合器内，延伸距离为阵列波导宽度
    y_span = L1
...
theta_buffer = deg2rad(10)
sector1 = mydraw.sector(radius=Ra,
                        theta_start=rad2deg(-da * (Narray/2 - 1/2) / Ra - theta_buffer + pi),
                        theta_stop=rad2deg(da * (Narray/2 - 1/2) / Ra + theta_buffer + pi),
                        x_center=2 * x_center, y_center=0,
                        angle_resolution=1.0, layer=(1, 0))
AWG.add_ref(sector1)
theta_span = dr * (N_channels/2 - 1/2) * 2 / Ra
sector2 = mydraw.sector(radius=Ra/2,
                        theta_start=0 + rad2deg(-theta_span - theta_buffer),
                        theta_stop=0 + rad2deg(theta_span + theta_buffer),
